...
def basis_function(x, k_order=3):
    """
...
def get_basic_function_data(order, k_order):
    """
    ...
    p_guass, M, px_guass = basis_function(Guass_point, k_order=k_order)
    p_end, _, _ = basis_function(x=1, k_order=k_order)
    p_begin, _, _ = basis_function(x=-1, k_order=k_order)
    p_center, _, _ = basis_function(x=0, k_order=k_order)
    ...
    return p_guass, px_guass, p_begin, p_end, M, Weight, p_center


if __name__ == '__main__':
    data = get_basic_function_data(order=9,k_order=3)
